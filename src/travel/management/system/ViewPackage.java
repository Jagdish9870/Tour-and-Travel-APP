...
//		contentPane.add(l1);
                ...
                try{
                    ...
                    while(rs.next()){
                        c4.add(rs.getString("username"));
                    }

                    rs.close();
                }catch(SQLException e){}

                c4.setBounds(271, 70, 150, 30);
                add(c4);
		...
		b2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
                            ...
                            try{
                                ...
                                ResultSet rs = c.s.executeQuery("select * from bookPackage where username = '"+s1+"'");
                                while(rs.next()){
...
//                }catch(SQLException e){}
		...
		btnExit.setBounds(100, 350, 120, 30);
                btnExit.setBackground(Color.BLACK);
                btnExit.setForeground(Color.WHITE);
		contentPane.add(btnExit);
                
                 JButton b3 = new JButton("previous bookings");
                b3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
                            
                            JDialog jd=new JDialog();
                            jd.setTitle("previous bookings");
                            jd.setBounds(200, 200, 1000, 500);  
                            jd.setVisible(true);
                            JTable table = new JTable();
		            table.setBounds(0, 100, 900, 350);
                            jd.add(table);
                            
                
                try{
                    Conn c = new Conn();
                        String displayCustomersql = "select * from bookPackage";
